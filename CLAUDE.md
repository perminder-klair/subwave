...
**Optional `analyzer` profile** — the acoustic-analysis sidecar (`profiles: ["analyzer"]`, `--profile analyzer up -d`), `subwave-analyzer` image (`docker/Dockerfile.analyzer` + `docker/analyzer/server.py`). It's the librosa + CLAP + Demucs analyzer **split out of tts-heavy** so an operator who only wants analysis (bpm/key/intro/loudness, "sounds-like" embeddings, vocal ranges) pulls a ~1.4GB image (or ~400MB lean with `--build-arg WITH_CLAP=0 WITH_DEMUCS=0`) instead of the ~6GB Chatterbox+PocketTTS one. Same `/health` + `/analyze` wire contract as tts-heavy and the **same `analyze_worker.py`** (single source of truth, copied into both images). The controller resolves its analysis backend from `config.analyzer.urls` — **`ANALYZE_URL` first, then `TTS_HEAVY_URL`** (`music/analyzer.ts` probes each `/health` for the `analyze` engine and uses the first that answers) — so existing `--profile tts-heavy` installs keep working with zero config because that image still carries the worker. The analyze worker therefore lives in **both** images for now.
