...
**Default-on `analyzer` service** — the acoustic-analysis sidecar (`subwave-analyzer` image, `docker/Dockerfile.analyzer` + `docker/analyzer/server.py`). **Starts by default** in all three composes (no profile gate — like controller/web); only `tts-heavy` (voices) stays opt-in. It's the librosa + CLAP + Demucs analyzer **split out of tts-heavy**: bpm/key/intro/loudness, "sounds-like" embeddings, vocal ranges. Published full (~1.4GB, CLAP+Demucs baked, amd64-only); build `--build-arg WITH_CLAP=0 WITH_DEMUCS=0` for a lean ~370MB librosa-only variant. Same `/health` + `/analyze` wire contract as tts-heavy and the **same `analyze_worker.py`** (single source of truth, copied into both images). The controller resolves its analysis backend from `config.analyzer.urls` — **`ANALYZE_URL` first, then `TTS_HEAVY_URL`** (`music/analyzer.ts` probes each `/health` for the `analyze` engine and uses the first that answers), then a local venv (`ANALYZE_PYTHON`) — so a `--profile tts-heavy`-only install still gets analysis (that image also carries the worker). The analyze worker lives in **both** sidecar images **and** the AIO. **AIO (`Dockerfile.aio`) bundles it** as a local venv (`ANALYZE_PYTHON=/opt/analyzer/venv/bin/python`, `HF_HOME` under the state dir) rather than a service, since the AIO has no sidecars.
