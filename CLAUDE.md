...
- **Every SCHEDULED spoken segment is a row in one slot table, driven by one per-minute cron.** `broadcast/talk-scheduler.ts` holds the table (hourly check, programme beats, banter, idents) and the pure planner over it; `scheduler.talkTick` is its only driver. The minutes are hand-partitioned for a reason (#310: an ident and an hourly check at :00 stacked two segments back to back), and a table is where that reason can be READ. Adding a fifth talk cron is the bug. Two things it deliberately does not own: eligibility, which still resolves through `dj-gate`/`listeners`/`dj-budget`/`programme` at fire time, and the :00 **session roll**, which is not a talk action and runs unconditionally before the planner — a muted or empty station must still roll. The segment director keeps its own `*/5` cron. → [`docs/internals/broadcast.md`](docs/internals/broadcast.md)
