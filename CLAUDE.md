...
- **Write `genres`, never `genre`** — the scalar column is GENERATED from `genres[0]`. `subsonic.songGenres()` is the single ingest normaliser, and it is also where the operator's **scene-consolidation rules** are applied, so a merge survives the next walk instead of being written back over. Genre matching is **one-directional**: a track's tag may refine a show's genre, never broaden it.
