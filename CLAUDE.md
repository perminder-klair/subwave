...
- `music/source.js` — the **music-source seam**. `getSource()` returns the active `MusicSource` (selected by `settings.source.provider`; env `MUSIC_SOURCE` wins), and the ~80 picker/request/queue/tagger call sites go through it — nothing imports `subsonic` directly any more. The interface + `Song` type + id helpers (`namespaceId`/`parseId`, prefixes `nd:`/`jf:`/`jam:`/`local:`) + the shared annotate builder + `sourceConfig()` live in `music/source-kit.js`; providers live under `music/sources/*.js` (`navidrome` wraps `subsonic.js`; plus `jamendo`, `jellyfin`, `local`). **Single active source** for now (no cross-provider merge; ids are namespaced so multi-source stays an additive follow-up). Methods are **non-optional** — unsupported capabilities return empty via `emptyDiscovery`, and the `capabilities` flags gate whether the picker/agent bother. **Provider-independent enrichment** is in `music/enrichment/{lastfm,lyrics}.js` (Last.fm crowd tags by artist name + LRCLIB lyrics), so every source gets rich embeddings, not just Navidrome. yt-dlp is deferred. See `docs/music-sources.md`.
- `music/subsonic.js` — Navidrome / Subsonic-compatible client (the `navidrome` provider's implementation), proper Subsonic salt+token MD5 auth (never plaintext). **`getAnnotatedUri(song)`** wraps the URI in `annotate:title=…,artist=…,subsonic_id=…:<uri>` so Liquidsoap reports metadata immediately (not waiting on ID3) and the `on_metadata` hook can recover the song id for `/cover/:id`. Also `getSimilarSongs`, `getArtistInfo`, `getTopSongs`, `getPlaylists`, `getRecentlyAddedAlbums`, `getFrequentAlbums`, `getCoverArtUrl`. **`isStationArchive(song)`** is the guard that keeps SUB/WAVE's own hourly archive mixdowns (path `archive/YYYY-MM-DD/HH-00.mp3`) out of selection/enumeration when a co-located Navidrome scans the archive dir — every song-returning function filters through it, so the picker/tagger/library UI never see junk `HH-00` "tracks" (issue #273). The `state/archive` dir should not be inside the Navidrome music library; the broadcast entrypoint also drops a `.ndignore` there as belt-and-suspenders.
...
- **Music source**: go through `getSource()` (`music/source.js`), never import `subsonic` directly — that's the seam that lets Jamendo/Jellyfin/local back the same pipeline. Keep using the provider's `getAnnotatedUri` for anything going to Liquidsoap — raw URLs lose metadata until ID3 arrives, and the `on_metadata` hook needs `subsonic_id` (the namespaced id) for `/cover/:id` artwork. New provider: add a `music/sources/<key>.js` implementing `MusicSource` (spread `emptyDiscovery` for the bits it lacks, set honest `capabilities`), register it in `source.js`, and surface it in the admin Settings "Music source" section.
