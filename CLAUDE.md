...
- **Library coverage never counts on the read path.** `coverage.total` is a `getAlbum` call per album — thousands of Navidrome requests on a real library — so `library-coverage.get()` only ever *reports* the last count, stamped `scannedAt`; the walk belongs to `refresh()`, which the operator's Count-library button (`POST /library/coverage/refresh`) and the runs that have already walked the catalogue are the only callers of. A staleness check inside `get()` is exactly how opening the admin Library page came to hammer Navidrome (#1570). The web side matches: coverage is polled only during a tagging run or an in-flight count, never on an idle tab. → [`docs/internals/music.md`](docs/internals/music.md)
