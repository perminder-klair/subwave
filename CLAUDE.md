...
- PocketTTS (kyutai-labs/pocket-tts) is the opt-in fifth engine — a 100M-param CPU-only model that runs ~6× real-time with ~200ms TTFB and ships built-in voices in English, French, German, Italian, Spanish and Portuguese. Same opt-in story as Chatterbox: the default controller image does NOT bundle it; build with `docker build --build-arg WITH_POCKETTTS=1` to create the venv at `/opt/pocket-tts/venv` and warm the Hugging Face cache. The dispatcher gates on `pocketTts.isAvailable()` (an `existsSync` on the venv interpreter), so a persona pointed at `pocket-tts` on a non-built image silently falls back to Piper rather than crashing. v1 exposes built-in voice ids only (`alba`, `anna`, `charles`, `estelle`, `giovanni`, `juergen`, `lola`, `rafael`) — the model also supports zero-shot cloning from a reference WAV, but the wrapper doesn't surface it yet. Voice list lives in `settings.POCKET_TTS_VOICES`; unknown ids passing the regex are forwarded to the worker, which falls back to its configured default if the model rejects them, so a stale persona value never silences a segment.
