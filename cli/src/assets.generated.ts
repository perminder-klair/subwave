...
# ANTHROPIC_API_KEY=
# OPENAI_API_KEY=
# Azure OpenAI. The key alone is not enough: the resource endpoint and the
# DEPLOYMENT name are settings, not env — admin UI → Settings → LLM. Paste the
# endpoint as its bare root (https://<resource>.openai.azure.com); only a
# resource pinned to a dated api-version needs the ?api-version=... query, and
# that legacy path cannot reach o-series / GPT-5.x deployments.
# If the deployment IS o-series / GPT-5.x, turn on "Reasoning deployment" on the
# same page — a deployment name cannot tell the station which model is behind it.
# The same key also covers Azure EMBEDDINGS for the library tagger, which need a
# deployment of their own (e.g. text-embedding-3-small) named under
# Settings → Library tagger → Embedding.
# AZURE_API_KEY=
