...
export async function pickNextTrack({ candidates, recentPlays, context, show = null, recentTransitions = [] }: {
  ...
  return djObject({
    ...
    schema: z.object({
      ...
      ...(fxActive ? {
        transition: z.enum(['normal', 'blend', 'sweep', 'washout', 'dissolve', 'chop']).nullable()
          .describe('transition treatment for this pick — "blend" only when the pair is exceptionally locked (near-identical tempo, close key; a plain crossfade already handles ordinary same-lane picks), "sweep" for a genuine gear-change, "washout" to ring this pick out as it ends, "dissolve" to melt the previous track into ambience under this pick, "chop" to cut the previous track out on the beat for an energy jump; "normal" or null for a plain crossfade. The TRANSITION EFFECTS guidance above explains each.'),
      } : {}),
    }),
    temperature: 0.5,
    kind: 'pickNextTrack',
