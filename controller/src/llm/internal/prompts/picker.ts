    ...
    schema: z.object({
      ...
      ...(fxActive ? {
        transition: z.enum(['normal', 'blend', 'sweep', 'washout', 'dissolve']).nullable()
          .describe('transition treatment for this pick — "blend" for a same-lane pick (similar tempo/energy/mood), "sweep" for a genuine gear-change, "washout" to ring this pick out as it ends, "dissolve" to melt the previous track into ambience under this pick; "normal" or null for a plain crossfade. The TRANSITION EFFECTS guidance above explains each.'),
      } : {}),
    }),
    temperature: 0.5,
    kind: 'pickNextTrack',
