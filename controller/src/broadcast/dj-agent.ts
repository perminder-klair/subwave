...
function effectsGuidance(): string {
  ...
  return `\n\nTRANSITION EFFECTS ("transition") — part of your craft, not a gimmick: a working DJ fires one every few songs when the moment earns it. Actively look for the moment on every pick; when you spot one, flag it — the station rate-limits and validates your choice against the audio analysis, so trust it to skip the ones that don't land rather than rationing them yourself:\n- "sweep": the track playing before your pick sinks under a slowly closing filter across the blend while your pick rises clean underneath. Use it whenever you make a genuine gear-change — a clear jump in energy, tempo, or mood (it only fires when the tracks measurably clash).\n- "washout": your pick dissolves into a pulsing, tempo-synced echo tail as it ENDS, ringing out into whatever follows. Use it to close a chapter: the last track of a themed run, heading into a talk break, or out of a dreamy/ambient track. Only an ordinary same-lane transition gets "normal" or null.`;
}
...
async function pickViaAgent(queue, { wantLink, audioWaypoint = null, current = null }: { wantLink: boolean; audioWaypoint?: number[] | null; current?: any }): Promise<boolean> {
  ...
  const queued = await enqueuePick(queue, song, object.reason, 'agent', link, current, { sweep, washout });
