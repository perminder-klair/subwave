...
class Queue {
  ...
  async airPendingVoice() {
    ...
    if (this.session.pendingHandoff()) {
  ...
  async airIntro(item: QueueItem, predecessor: Track | null = null) {
    ...
    try {
      ...
      await airVoice(targetFile, item.introWav, item.introScript || '', voiceGainDb(kind, item.introPersona || undefined), { jingleFile: this.paths.liquidsoap.jinglePlayingFile, chainKey: this.channelId });
      this.persist();
    ...
    if (this.autoPick && this.upcoming.length === 0 && !this.pickerBusy && callsAllowed) {
      ...
      (async () => {
        try {
          ...
          const ctx = await this.stationContext(showAt ?? undefined);
