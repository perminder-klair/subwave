...
// The persistent right rail — the sentence-based order editor ("Put Chill
// Lounge on the air from 16:00 until 18:00"), the orders behind the current
// listing, the week's numbers, computed suggestions, and airtime-vs-target
// bars. All of it renders from the same grid the board and listing use.
...
export default function RightRail({
  ...
  orderNo, onJumpToDay, orders, showById, onPickOrder,
  stats, suggestions, onDismissSuggestion, airtime, tickPct, target,
}: RightRailProps) {
  const lineShow = shows.find(s => s.id === lineShowId) ?? null;
  return (
    <aside className="min-w-0 self-stretch border-l border-ink bg-[var(--page-bg)] p-[22px]">
      {/* ── Editing this line ─────────────────────────────────────────── */}
      ...
      <div className="mt-2.5 flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => onJumpToDay(line.day)}
          className="ml-auto cursor-pointer border-0 bg-transparent p-0 font-mono text-[8.5px] tracking-[0.16em] text-muted uppercase hover:text-ink"
        >
          Jump to it in the listing →
        </button>
      </div>

      {/* ── Orders behind this listing ────────────────────────────────── */}
      <div className="mt-[22px] border-t border-ink pt-[18px]">
        <div className="mb-0.5">
          <span className="eyebrow text-ink">Orders behind this listing</span>
        </div>
      ...
      <div className="mt-[22px] border-t border-ink pt-[18px]">
        ...
        <div className="grid grid-cols-2 gap-[11px]">
          <RailStat n={stats.booked} label="hours booked" />
          <RailStat n={168 - stats.booked} label="hours of dead air" accent />
          <RailStat n={stats.showCount} label="shows on rotation" />
          <RailStat n={stats.orderCount} label="standing orders" />
        </div>
      </div>
...
function RailStat({ n, label, accent }: { n: number; label: string; accent?: boolean }) {
  return (
    <div
      className={cn(
