...
# Edge mode — decides which API/stream URLs get baked into the web build.
#   caddy (default) : web sits behind the Caddy edge proxy, so the web app's
#                     same-origin defaults (/api, /stream.mp3) resolve.
#   none            : no Caddy — the browser reaches the controller (:7701)
#                     and icecast (:7702) on their own ports, so the build
#                     needs absolute URLs. SUBWAVE_HOST is the hostname the
#                     browser will use to reach this box (default: localhost;
#                     set it to the LAN IP / DNS name for off-box listeners).
SUBWAVE_EDGE="${SUBWAVE_EDGE:-caddy}"
SUBWAVE_HOST="${SUBWAVE_HOST:-localhost}"

say()  { printf '\033[1;36m==>\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33m[!]\033[0m %s\n' "$*" >&2; }
die()  { printf '\033[1;31m[x]\033[0m %s\n' "$*" >&2; exit 1; }

case "$SUBWAVE_EDGE" in
  caddy|none) ;;
  *) die "SUBWAVE_EDGE must be 'caddy' or 'none' (got: '$SUBWAVE_EDGE')" ;;
esac

# Caddy fronts the web server on loopback; without Caddy the browser hits the
# web server directly, so it has to bind all interfaces.
if [[ "$SUBWAVE_EDGE" == none ]]; then WEB_HOSTNAME=0.0.0.0; else WEB_HOSTNAME=127.0.0.1; fi
...
# API/stream URLs have to be decided here — a runtime Environment= in the
# systemd unit / launchd plist can't change an already-built bundle. Write
# web/.env.production (Next loads it automatically for `npm run build`) so the
# build matches the chosen edge mode without the operator hand-editing env.
WEB_ENV="$REPO_DIR/web/.env.production"
if [[ "$SUBWAVE_EDGE" == "none" ]]; then
  say "Edge mode 'none' — baking absolute API/stream URLs into the web build (host: $SUBWAVE_HOST)"
  cat > "$WEB_ENV" <<EOF
# Auto-generated by scripts/install-native.sh — SUBWAVE_EDGE=none.
# No Caddy edge: the browser reaches the controller and icecast directly, so
# the web app's same-origin defaults (/api, /stream.mp3) would 404. Re-run the
# installer to regenerate; delete this file and rebuild for same-origin.
NEXT_PUBLIC_API_URL=http://${SUBWAVE_HOST}:7701
NEXT_PUBLIC_STREAM_URL=http://${SUBWAVE_HOST}:7702/stream.mp3
EOF
elif [[ -f "$WEB_ENV" ]] && grep -q 'install-native.sh' "$WEB_ENV" 2>/dev/null; then
  # Caddy edge — same-origin defaults are correct. Drop a stale file a previous
  # SUBWAVE_EDGE=none run generated so this rebuild reverts cleanly. Only files
  # this installer wrote (matched by the marker comment) are removed.
  say "Edge mode 'caddy' — removing stale installer-generated $WEB_ENV"
  rm -f "$WEB_ENV"
fi
...
export STATE_DIR SOUNDS_DIR ICECAST_BIN LIQUIDSOAP_BIN NODE_BIN CADDY_BIN TSX_BIN \
       PIPER_BIN PIPER_VOICE PIPER_VOICE_CONFIG WEB_HOSTNAME \
...
Native install ready.
  ...
  Repo      : $REPO_DIR
  Piper     : $PIPER_BIN
  Edge      : $SUBWAVE_EDGE$([[ "$SUBWAVE_EDGE" == none ]] && echo " (web build → http://$SUBWAVE_HOST:7701 + :7702)")
EOF
if [[ "$OS" == "Linux" ]]; then
